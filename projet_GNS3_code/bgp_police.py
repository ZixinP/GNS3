                ...
                for interface in router.interfaces_physiques:
                    if interface.neighbor_id != None:
                        ...
                        if interface.neighbor_as in AS.customers.keys():
                                ...
                                dict["access-list"].append([
                                    f"ipv6 access-list 100 deny ipv6 any any permit {as_number}:{AS.provider_com}",
                                    f"ipv6 access-list 200 deny ipv6 any any permit {as_number}:{AS.peer_com}",
                                    f"ipv6 access-list 300 permit ipv6 any any permit {as_number}:{AS.customer_com}",
                                    f"ipv6 access-list 400 permit ipv6 any any",
                                    ])
                                ...
                                dict["BGP"]["neighbor active"].append(f" neighbor {nei_add} route-map Advertize out")
                                        ...
                                        # filtrer les routes pour que le voisin reçoit seulement les routes de customer et les routes de ce AS
                                        dict["access-list"].append([
                                            f"ipv6 access-list 100 deny ipv6 any any permit {as_number}:{AS.customer_com}",
                                            f"ipv6 access-list 200 deny ipv6 any any permit {as_number}:{AS.peer_com}",
                                            f"ipv6 access-list 300 permit ipv6 any any permit {as_number}:{AS.provider_com}",
                                            f"ipv6 access-list 400 permit ipv6 any any",
                                            ])
